"""
Tenant Guard Middleware — enforces tenant status, usage limits, and plan features.
...
PLAN_LIMITS = {
    "trial": {"queries": 100, "documents": 20, "staff": 2, "policyholders": 50, "features": ["widget"]},
    "starter": {"queries": 1000, "documents": 100, "staff": 5, "policyholders": 500, "features": ["widget", "batch_upload"]},
    "professional": {"queries": 10000, "documents": 500, "staff": 20, "policyholders": 5000, "features": ["widget", "batch_upload", "api_access"]},
...
FEATURE_GATES = {
    "batch_upload": ["/api/v1/policies/upload-batch", "/api/v1/communications/upload-batch"],
}
...
UPLOAD_PATHS = ["/api/v1/policies/upload", "/api/v1/communications/upload"]
...
class TenantGuardMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if tenant_info.get("status") == "suspended":
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "detail": "Your agency account has been suspended. Please contact support.",
                },
            )
        ...
        if any(path.startswith(up) for up in UPLOAD_PATHS) and request.method == "POST":
    ...
    async def _get_tenant_info(self, tenant_id: str) -> dict | None:
        ...
        try:
            ...
            logger.error(f"Tenant guard DB error: {e}")
            return None
        ...
        try:
            ...
            async with async_session() as db:
                ...
                if count >= query_limit:
                    logger.warning(f"Tenant {tenant_id} exceeded query limit: {count}/{query_limit}")
                    return JSONResponse(
                        status_code=429,
                        content={
                            "error": "Query limit exceeded",
                            "detail": f"Your plan allows {query_limit} queries per month. "
                                      f"You've used {count}. Please upgrade your plan or wait until next month.",
                            "usage": {"used": count, "limit": query_limit},
                        },
                    )
        except Exception as e:
            logger.error(f"Query limit check error: {e}")
        return None
    ...
    async def _check_document_limit(self, tenant_id: str, plan: dict) -> JSONResponse | None:
        ...
        try:
            ...
            async with async_session() as db:
                ...
                if count >= doc_limit:
                    logger.warning(f"Tenant {tenant_id} exceeded document limit: {count}/{doc_limit}")
                    return JSONResponse(
                        status_code=429,
                        content={
                            "error": "Document limit exceeded",
                            "detail": f"Your plan allows {doc_limit} documents. "
                                      f"You have {count}. Please upgrade or delete unused documents.",
                            "usage": {"used": count, "limit": doc_limit},
                        },
                    )
        except Exception as e:
            logger.error(f"Document limit check error: {e}")
        return None
